...
export function hasMemoryRetrievalIntent(message = '') { return MEMORY_INTENT.test(String(message)); }
export function runAtlasCore({ message = '', identity = {}, requestedTool = null, memoryAvailable = true } = {}) {
  ...
  return {
    ...
    memoryPlan: { action: shouldSearchMemory ? 'SEARCH' : 'NONE', availability: memoryAvailable ? 'PERSISTENT_SOURCE_BACKED' : VERIFICATION_STATES.NOT_CONFIGURED },
